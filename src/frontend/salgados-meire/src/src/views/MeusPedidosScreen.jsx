          ...
          <div className="card__container">
            {
              isLoading ? <Spinner /> : (
                <>
                  <Button colorScheme='blue' onClick={getPedidos}>Atualizar Página</Button>
                  {
                    meusPedidos.map((pedido) => (
                      <div key={pedido.id}>
                        <Card className="card">
                          <CardHeader>
                            <Heading size='md'>Data: {moment(pedido.dataPedido).format('DD-MM-YYYY')}</Heading>
                          </CardHeader>
                          <CardBody>
                            <p>Status do pedido: <Tag>{status[`${pedido.status}`]}</Tag></p>
                            <Divider className="divider" />
                            <Heading size='sm'>Detalhes do pedido:</Heading>
                            <p>Forma de pagamento: {formaPagamento[`${pedido.formaPagamento}`]}</p>
                            {
                              pedido.trocoPara > 0 &&
                              <p>Troco para: R$ {pedido.trocoPara}</p>
                            }
                            <Heading size='sm' className="heading">Itens do pedido:</Heading>
                            {
                              pedido.itensPedido.map((item) => (
                                <div key={item.id}>
                                  <p>{item.salgado.nome} x{item.quantidade} - R$ {item.valorUnitarioNaCompra}</p>

                                </div>
                              ))
                            }
                            <Heading size='md' className="heading">Total do pedido: R$ {pedido.itensPedido.reduce((acc, item) => acc + (item.valorUnitarioNaCompra * item.quantidade), 0)}</Heading>
                          </CardBody>
                        </Card>
                      </div>
