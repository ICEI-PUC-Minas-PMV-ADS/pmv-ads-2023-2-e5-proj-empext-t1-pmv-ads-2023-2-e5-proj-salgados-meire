...
const RelatorioScreen = () => {
  ...
  return (
    <>
      <div>
        <Header title={"Relatório"} showProfileIcon={false} showCartIcon={false} />
      </div>
          ...
          <Card>
            <CardBody className="card_body">
              ...
              <div className="select_container">
                <Select placeholder="Selecione o mês" onChange={(e) => onChangeMonthValue(e.target.value)}>
                  {months.map((mes, index) => <option key={`${index}-${mes.value}`} value={mes.value}>{mes.label}</option>)}
                </Select>
              </div>
